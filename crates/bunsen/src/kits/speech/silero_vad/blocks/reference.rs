...
mod tests {
    ...
    fn test_real_data() {
        ...
        let mut s_ctx = Tensor::zeros([1, 64], &device);
        let mut r_ctx = Tensor::zeros([1, 64], &device);

        for (i, chunk) in input.chunks_exact(512).enumerate() {
            println!("Iter {i}");
            let input = Tensor::<B, 1>::from_floats(chunk, &device).unsqueeze_dim(0);
            (s_out, s_state, s_ctx) = s_mod.forward(input.clone(), sample_rate, s_state, s_ctx);
            (r_out, r_state, r_ctx) = r_mod.forward(input.clone(), sample_rate, r_state, r_ctx);
